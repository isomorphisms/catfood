...
Actual standalone DEX bytecode is separate under `android/dex/`. An APK that happens to contain only DEX/ART bytecode is still an APK and therefore belongs in both compatible device APK shelves, not in the DEX directory.
...
`android/dex/` is generated by the same pinned ai-ci collector. A retained file there must itself begin with DEX magic (`dex\n`). Sources may be standalone `.dex` release assets or DEX members extracted from ZIP/TAR release artifacts of repositories explicitly listed in `android/dex-archive-repositories.txt`. Its generated `manifest.tsv` records repository, release, asset/member, checked-in filename, SHA-256, byte count, and source URL.
