...
prepare_grease() {
    name=$1
    prepare_archive "$name" "$payload/$GREASE_FILE" "$GREASE_SHA256" || return $?
    engine="$work/$name/bin/ysh"
    [ -x "$engine" ] || {
        printf 'missing executable %s\n' "$engine"
        return 1
    }
    # oils-for-unix is a multicall binary.  OSH syntax is selected by argv[0].
    # Keep the YSH engine unchanged and expose a sibling OSH name only inside
    # this temporary acceptance worktree.
    ln -sf ysh "$work/$name/bin/osh" || return $?
}
...
grease_readable_case() {
    prepare_grease grease-readable || return $?
    osh="$work/grease-readable/bin/osh"
    printf 'parser_mode=osh\n'
    output=$($osh "$payload/grease-readable-smoke.ysh" 2>&1)
    status=$?
    printf '%s\n' "$output"
...
grease_ib_case() {
    ...
    prepare_grease grease-ib || return $?
    ysh="$work/grease-ib/bin/ysh"
    printf 'parser_mode=ysh\n'
...
cloud_storage_help_case() {
    ...
    prepare_grease grease-cloud || return $?
    osh="$work/grease-cloud/bin/osh"
    printf 'parser_mode=osh\n'
    printf 'cloud_storage_ref=%s\n' "$CLOUD_STORAGE_REF"
    output=$($osh "$payload/$CLOUD_STORAGE_FILE" --help 2>&1)
    status=$?
    printf '%s\n' "$output"
    [ "$status" -eq 0 ] || return "$status"
...
reddit_case() {
    ...
    # Android 14 requires dynamically loaded code to be read-only before the
    # runtime opens it.  The cloud-produced archive is immutable by digest, but
    # tar extraction makes files owner-writable unless we close that permission.
    chmod 0444 "$dex" "$library" || return $?
    printf 'android_sdk=%s\n' "$(/system/bin/getprop ro.build.version.sdk 2>/dev/null || printf unknown)"
    ls -l "$dex" "$library"
...
EOF_FIXTURE
    ...
    for name in az abe; do
        printf '#!/system/bin/sh\n# catfood az wrapper\nexit 99\n' > "$workspace/bin/$name"
        chmod +x "$workspace/bin/$name"
    done
    printf '#!/system/bin/sh\n# catfood aa wrapper\nexit 99\n' > "$workspace/bin/aa"
    chmod +x "$workspace/bin/aa"
...
run_case grease_readable_osh grease_readable_case
if [ "${GREASE_IB:-0}" = 1 ]; then
    run_case grease_ib_ysh_mapped_index grease_ib_case
fi
run_case cloud_storage_osh_help cloud_storage_help_case
